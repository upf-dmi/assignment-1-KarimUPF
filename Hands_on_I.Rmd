...
plot_A <- ggplot(Table_1, aes(x = Age)) +
  geom_histogram(
    binwidth = 10, # Adjust the bin width to match your desired resolution
    fill = "lightcyan2",
...
wrap_elements(panel = as.ggplot(~hist(
  Table_1$Age, 
  ylim = c(0, 50), 
  main = "Age", 
  xlab = "Age (years)", 
  ...
  plot_B + # Plot B
  wrap_elements(panel = as.ggplot(~barplot(
  names = bar_df$names,
  ...
  main = "Clinical classification" # Plot C
)), clip = FALSE) +
  wrap_elements(panel = as.ggplot(~barplot(

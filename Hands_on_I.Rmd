...
plot_Age <- ggplot(data = heart_df_filtered, aes(x = Age)) + 
  geom_histogram(aes(y = after_stat(density)), fill = 'blue1', color = 'black', binwidth = 5) +
...
plot_Peak <- ggplot(data = heart_df_filtered, aes(x = Peak)) + 
  geom_histogram(aes(y = after_stat(density)), fill = 'blue1', color = 'black', binwidth = 0.1) +
  geom_density(fill = '#F0FFFF', alpha = 0.5) +
...
# Cargar datos
Table_2 <- read_excel("Article/Table 2.XLSX", skip = 1, na = "NI")
